...
def get_word_counts(opt, count_inputs):
    ...
    while True:
        ...
        if count_inputs:
            input_ = world.acts[0]['text']
            words = input_.split()
            word_counter.update(words)
            total_count += len(words)
            all_utts.append(input_)
...
def learn_arora(opt):
    """
    Go through data and collect word counts, thus compute the unigram
    ...
    """
    arora_file = os.path.join(opt['datapath'], opt['arora_data_dir'], 'arora.pkl')

    opt['task'] = 'fromfile:fbformat'
    opt['log_every_n_secs'] = 2

    print('Getting word counts from train set...')
    opt['datatype'] = 'train:ordered'
    opt['fromfile_datapath'] = os.path.join(
        opt['datapath'], opt['arora_data_dir'], 'train.txt'
    )
    word_counter_train, total_count_train, all_utts_train = get_word_counts(
        opt, count_inputs=opt['include_context'])

    print('Getting word counts from val set...')
    opt['datatype'] = 'valid'
    opt['fromfile_datapath'] = os.path.join(
        opt['datapath'], opt['arora_data_dir'], 'valid.txt'
    )
    ...
    # Make dict mapping dataset utterances to Arora sent emb
...
def load_arora(opt):
    """
    Load the data in the arora.pkl file in data/opt['arora_data_dir']
    """
    arora_fp = os.path.join(opt['datapath'], opt['arora_data_dir'], 'arora.pkl')
